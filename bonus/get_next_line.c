...
char	*ft_handle(char **full_buff, int index)
{
	...
	line = ft_substrr(*full_buff, 0, index + 1);
	temp = ft_substrr(*full_buff, index + 1,
			ft_strlenn(*full_buff) - index - 1);
	ft_free(full_buff);
	*full_buff = temp;
	return (line);
}
...
char	*get_line(int fd, char **full_buff)
{
	...
	while (ft_charlen(*full_buff) == -1)
	{
		...
		buffer[bytes_read] = '\0';
		*full_buff = ft_strjoinn(*full_buff, buffer);
	}
